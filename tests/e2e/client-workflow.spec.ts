...
async function deleteCustomizedTemplatePart( page: Page ): Promise< void > {
	const response = await page.request.delete(
		'/wp-json/wp/v2/template-parts/site-theme%2F%2Fsite-header',
		{
			params: { force: true },
	...
	expect(
		[ 200, 404 ],
		'could not restore the site header template part'
	).toContain( response.status() );
}

async function deleteCustomizedTemplate(
	page: Page,
	slug: string
): Promise< void > {
	const response = await page.request.delete(
		`/wp-json/wp/v2/templates/site-theme%2F%2F${ slug }`,
		{
			params: { force: true },
			headers: await restHeaders( page ),
		}
	...
	expect(
		[ 200, 400, 404 ],
		`could not restore the ${ slug } template`
	).toContain( response.status() );
}
...
test( 'client_editor edits header text and saves', async ( { page } ) => {
	...
	try {
		...
		await expect(
			canvas.getByText( 'Edited by the client', { exact: true } )
		).toBeVisible();
...
test( 'client_editor adds a core block, reorders it, then removes it', async ( {
	page,
} ) => {
	await loginAs( page, CREDS.clientEditor.u, CREDS.clientEditor.p );
	...
	try {
		...
		const stored = await page.request.get(
			'/wp-json/wp/v2/templates/site-theme%2F%2F404?context=edit',
			{ headers: await restHeaders( page ) }
		);
		...
		await expect( canvas.getByText( marker, { exact: true } ) ).toHaveCount(
			0
		);
		await expect(
			canvas.getByText(
				'The page you were looking for could not be found. Try a search instead.',
				{ exact: true }
			)
		).toBeVisible();
...
test( 'client_editor changes a Global Styles colour and reaches a typography setting', async ( {
	page,
} ) => {
	await loginAs( page, CREDS.clientEditor.u, CREDS.clientEditor.p );
...
test( 'client_editor can undo a Site Editor change', async ( { page } ) => {
	...
	await expect( canvas.locator( '.editor-styles-wrapper h1' ) ).toHaveText(
		'Nothing here'
	);
} );
...
test( 'client_editor can edit the navigation menu', async ( { page } ) => {
	...
	try {
		...
		// REST endpoint after opening the real navigation editor because the
		// editor data layer serialises its own block state over raw content.
...
test( 'every static section of the demo page is visible in the editor canvas', async ( {
	page,
} ) => {
	await loginAs( page, CREDS.clientEditor.u, CREDS.clientEditor.p );
	...
	await expect(
		canvas.locator( '.editor-styles-wrapper h1.wp-block-heading' ).filter( {
			hasText:
				'Ship client sites without losing the design to a database',
		} )
	).toBeVisible();
	await expect(
		canvas.locator( '.editor-styles-wrapper .wp-block-columns' ).first()
	).toBeVisible();
	await expect(
		canvas.locator( '.editor-styles-wrapper .wp-block-image img' )
	).toBeVisible();
	await expect(
		canvas.locator( '.editor-styles-wrapper .wp-block-button' ).first()
	).toBeVisible();
	await expect(
		canvas.locator(
			'.editor-styles-wrapper .reference-callout__testimonial--preview'
		)
	).toBeVisible();
} );
