...
async function deleteCustomizedEntity( page: Page, endpoint: 'templates' | 'template-parts', id: string ): Promise< void > {
	const response = await page.request.delete( `/wp-json/wp/v2/${ endpoint }/${ encodeURIComponent( id ) }`, {
		params: { force: true },
		headers: await restHeaders( page ),
		timeout: 10_000,
	} );

	expect( [ 200, 404 ], `could not restore ${ endpoint }/${ id }` ).toContain( response.status() );
}
...
test( 'client_editor can edit and save a template, and the change persists', async ( { page } ) => {
	...
	const canvas = await openSiteEditorCanvas( page, '/wp_template/site-theme//page' );

	try {
		await canvas.locator( '.editor-styles-wrapper main h1, .editor-styles-wrapper main p' ).first().click();
		...
		const stored = await page.request.get( '/wp-json/wp/v2/templates/site-theme%2F%2Fpage?context=edit', { headers: await restHeaders( page ) } );
...
test( 'client_editor can edit and save the header template part, and the change persists', async ( { page } ) => {
	...
	try {
		...
		await expect( canvas.getByText( marker, { exact: true } ) ).toBeVisible();

		await saveInEditor( page );

		const stored = await page.request.get( '/wp-json/wp/v2/template-parts/site-theme%2F%2Fsite-header?context=edit', { headers: await restHeaders( page ) } );
		expect( stored.status() ).toBe( 200 );
		expect( JSON.stringify( await stored.json() ) ).toContain( marker );

		await page.goto( '/' );
		await expect( page.locator( 'header.site-header' ) ).toContainText( marker );
	} finally {
		await deleteCustomizedEntity( page, 'template-parts', 'site-theme//site-header' );
	}
} );
