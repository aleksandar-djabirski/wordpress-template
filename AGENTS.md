...
- `scripts/generate-block-index`, `scripts/check-database-overrides`, `scripts/sanitize-database`, `scripts/verify-environment`, `scripts/rename-project`, `scripts/enable-commerce` — the sanitize/check/verify-env three are thin wrappers around `wp agency check-overrides|sanitize|verify-env`. `check-overrides` is now a compatibility alias that prints a deprecation warning — prefer `wp agency state-diff`, which is the machine-readable form.
